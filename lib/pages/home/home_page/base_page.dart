  ...
  Widget build(BuildContext context) {
    ...
    // FutureBuilder<void>(
      ...
      //    } else {
            ...
            ChangeNotifierProvider<PersonalDashboardViewModel>(
              create: (context) => PersonalDashboardViewModel(),
              child: Consumer<ThemeManager>(
                builder: (context, themeManager, child) => 
                Consumer<PersonalDashboardViewModel>(
                  builder: (context, model, child) => 
                  FutureBuilder(
                    future: model.updateProfile(),
                    builder: (BuildContext context, AsyncSnapshot<void> snapshot) {
                      if (snapshot.connectionState == ConnectionState.done) {
                        if (snapshot.hasError) {
                          if (snapshot.error is GroupingProjectException) {
                            if ((snapshot.error as GroupingProjectException).code ==
                                GroupingProjectExceptionCode.notExistInDatabase) {
                                DataController().createUser(userProfile: AuthService().getProfileName());
                              // TODO : 拿到第三方認證 Profile 後，將資料傳入 createProfile
                            }
                            return Scaffold(body: NotFoundPage.fromError("ERROR"));
                          } else {
